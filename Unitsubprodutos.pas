...
begin
  try
    ...
    Application.MessageBox('Produto já Cadastrado!', 'Sub-Produto', mb_Ok + mb_IconExclamation);
  end;
  DBEdit2.SetFocus;
...
procedure Tfrmsubprodutos.BitBtn5Click(Sender: TObject);
begin
DM.TsubProdutos.Open;
if DM.TsubProdutos.RecordCount = 0 then
begin
DM.TsubProdutos.Close;
Application.MessageBox('Não hà sub-produto para ser excluído!', 'Informação', mb_Ok + mb_IconInformation);
end;

DM.TsubProdutos.Open;
if DM.TsubProdutos.RecordCount <> 0 then
begin
If Application.MessageBox('Confirma Exclusão deste sub-produto?', 'Confirmação',
mb_YesNo + mb_ICONQUESTION) = idYes then
begin
DM.TsubProdutos.Delete;
DM.TsubProdutos.Refresh;
